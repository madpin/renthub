...
from models import Song, Listing, Image
from database import async_session, get_session
from models import ImageWithRelationship, ListingWithRelationship
...
@app.get("/listings", response_model=List[ListingWithRelationship])
async def get_listings(session: AsyncSession = Depends(get_session)):
    result = await session.execute(select(ListingWithRelationship))
